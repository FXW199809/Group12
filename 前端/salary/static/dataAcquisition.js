// var url = "../../static/axiosIntegration";
// function addScript(url){
// 	document.write("<script language=javascript src="+url+"></script>");
// }
// import { axiosIntegration } from "../../static/axiosIntegration";
var dataAcquisition = {
    store:{ //配置项
      storeVer     : '1.0.3',     //版本号
      ...
      sendUrl      : "http://localhost:8080/pushlog",   //log采集地址（需配置）
    ...
    util: { //工具函数
      ...
      getCookie: function (name) {
        //表示匹配以' '或者name开头的字符串，并且name的=后面有值（不是；）
        ...
        if (dataAcquisition.store.useStorage) {
          ...
          if (arr = document.cookie.match(reg)) {
            return (decodeURIComponent(arr[2]));//decodeURIComponent() 函数可对 encodeURIComponent() 函数编码的 URI 进行解码
          } else {
            return null;
          }
        }
    ...
    init: function () {
      ...
      if(this.store.openInput){
        ...
        for (var i = 0; i < _ACIDoms.length; i++) {
          ...
          //如果input元素的类型在acrange范围之内，就添加监听事件
      ...
      if(this.store.openClick){
        ...
        document.addEventListener("click", function (e) {
          ...
          //获得触发事件对象，srcElement对应IE，target对应其他浏览器
    ...
    bindAjaxHook: function () {//对ajax中的异常进行捕获,需将代码置于业务代码之前，对所有请求进行代理
      //初始化一个请求
      var nativeAjaxOpen = XMLHttpRequest.prototype.open(null,null,false);
      ...
      var proxyXhrObj ={
        open: function() {
          ...
          //apply用于传递数组中的参数，this表示外层这个open方法
      ...
      // console.log("200" + XMLHttpRequest.prototype.readyState + "200");
      // if(XMLHttpRequest.prototype.send === 200){
      //   hhh();
      // }
    },
    ...
    postData: function () {//数据上报
      var _this = this,
        ...
        storePath = window.location.href,
      ...
      console.log(JSON.stringify({uuid: uuid, acData: data}));
  ...
  //如果AMD环境，就定义为AMD模块；如果是一般环境，就直接运行将返回值赋值给window['dataAc']
